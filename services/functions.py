...
class FunctionManager:    
    ...
    async def create_avatar(self, json_data: str) -> Dict[str, Any]:
        try:
            ...
            [ПРАВИЛА_СОЗДАНИЯ_ПРОМПТА]
            При создании промта ты учитываешь 90 процентов из этих данных: age, sex, description
            А также на остальные 10 процентов из этих данных: interests, abilities, places, additionalDetails

            В промпте ты должен отразить:
            ...
            Примеры промптов:
            - "A young man with short brown hair and blue eyes, wearing a blue t-shirt and jeans, standing in a park"
            - "A young woman with long blonde hair and green eyes, wearing a red dress and high heels, standing in a city street"
            - "A young man with short black hair and brown eyes, wearing a black t-shirt and jeans, standing in a park"
            - "A young woman with long brown hair and blue eyes, wearing a blue dress and high heels, standing in a city street"
            - "A young man with short blonde hair and green eyes, wearing a green t-shirt and jeans, standing in a park"
            - "A young woman with long brown hair and blue eyes, wearing a blue dress and high heels, standing in a city street"

            Промпту стоит выглядеть след. образом: f"{пол}, {возраст: ребёнок | подросток | взрослый | пожилой}, {цвет и длина волос}, {стрижка/причёска}, {цвет глаз}, {выражение лица: улыбается | серьёзный | удивлённый}, {аксессуары: шляпа | шарф | серёжки | ничего}, {эмоция или настроение}"
...
- add_face_to_db(text: str): Сохранить лицо с изображения в базу данных. Используется что бы запомнить лицо человека и использовать его для ответа пользователю. Обычно нужно сохранять лицо сообщеает что это мой друг и тд и скидывает фотографию.
